...
const inpText = document.querySelector(".wrapper .inputText")
...
const wpmRes = document.querySelector(".wpmResult span")
const mistakesRes = document.querySelector(".mistakesResult span")
const accuracyRes = document.querySelector(".accuracyResult span")
...
function loadParagraph() {
    ...
    typingText.addEventListener("click", () => inpText.focus());
}

// Check if the user's typing matches the expected text
function Typing() {
    let characters = typingText.querySelectorAll("span");
    let typedChar = inpText.value.split("")[indexOfChar];
    // If there are still characters to type and time has not run out
    if (indexOfChar < characters.length - 1 && timeRem > 0) {
        ...
        if (typedChar == null) {
            if (indexOfChar > 0) {
                ...
                if (characters[indexOfChar].classList.contains("incorrect")) {
                    numOfMistakes--;
                }
                // Remove the "correct" and "incorrect" classes from the previous character
                characters[indexOfChar].classList.remove("correct", "incorrect");
            }
        // If the user types a character, check if it is correct
        } else {
            if (characters[indexOfChar].innerText == typedChar) {
                characters[indexOfChar].classList.add("correct");
            } else {
                // If the user typed the wrong character, increment the mistake count
                numOfMistakes++;
                characters[indexOfChar].classList.add("incorrect");
            }
            indexOfChar++;
        }
        // Remove the "active" class from all characters and add it to the current character
        characters.forEach(span => span.classList.remove("active"));
        characters[indexOfChar].classList.add("active");
    ...
    // If the user has typed all the characters or time has run out
        ...
        finalAccuracy = Math.round((indexOfChar - numOfMistakes) / indexOfChar * 100);
        // Display the final WPM, mistakes, and accuracy to the user
        wpmRes.innerText = finalWPM;
        mistakesRes.innerText = finalMistakes;
        accuracyRes.innerText = finalAccuracy;
...
function calcWPM() {
    let wpm = Math.round(((indexOfChar - numOfMistakes) / 5) / (maxTime - timeRem) * 60);
...
function resetGame() {
    ...
    clearInterval(timer);
    timeRem = maxTime;
    indexOfChar = numOfMistakes = isTyping = 0;
...
loadParagraph();
inpText.addEventListener("input", Typing);
